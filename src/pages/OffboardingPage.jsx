      ...
      <div className="mt-4 flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <CompleteButton
            ...
            onClick={() => offStatusChange(t.id, "completed")}
          />
        </div>
      </div>
    </article>
...
export default function OffboardingPage() {
  ...
  const fetchTasks = async () => {
    ...
    setError("");
    try {
      const res = await axios.get(`${API_BASE_URL}/api/offboarding`, { headers });
      setRows((res.data?.tasks || []).map(formatTask));
      setPage(1);
    } catch (err) {
      setError(err?.response?.data?.error || "Failed to load offboarding tasks");
    } finally {
      setLoading(false);
    }
  };
  ...
  async function updateStatus(id, status) {
    try {
      await axios.patch(
        `${API_BASE_URL}/api/offboarding/${id}/status`,
        { status },
        { headers }
      );
      setRows((prev) =>
  ...
  async function deleteTask(id) {
    try {
      await axios.delete(`${API_BASE_URL}/api/offboarding/${id}`, { headers });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h2 className="text-xl font-semibold">Offboarding Tasks</h2>
          <p className="text-sm text-gray-600">
            Track new employee offboarding progress
          </p>
        </div>
